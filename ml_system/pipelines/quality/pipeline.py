...
class QualityPipeline:
    ...
    def _choose_split(self, index: int, total: int) -> str:
        train_ratio, val_ratio, _ = self._config.split_ratios
    ...
    def run(self, rows: Iterable[dict]) -> QualityReport:
        ...
        min_count = min((len(bucket) for bucket in label_buckets.values()), default=0)
        ...
        for _, bucket in balanced_grouped.items():
